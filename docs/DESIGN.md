...
チラシの特売品から、**平日のうち3日（月・水・金）の夕食**を組む。土日に1回買い物へ
行き、残りの2日は作らない。
...
| | |
|---|---|
| 献立 | 3日ぶんの夕食。1日につき本命1つと代替2つ |
| 買い物リスト | 土日に買うもの。2人分・3日ぶん。冷凍する指示つき |
...
最初は平日5日で組んだ。実際に1週ぶん出してみて「**5日間自炊はつらい**」と返って
きたので3日にした。作らない日を後から諦めるのではなく、**最初から織り込む。**

5日で組むと、作れなかった日はただの失敗として残る。3日で組めば、残りの2日は
外食でも惣菜でも構わない日になる。献立が守れる状態を先に作るほうが、記録まで
繋がる。

**月・水・金にする。** 2日続けて作らずに済む。月火水だと3日続きになって、
5日から減らした意味が薄い。
...
画面は月〜金を全部並べ、品の無い日に「作らない日」と出す。週の形が見えるほうが、
買い物と冷凍の段取りが読めるため。

**欠落と見分けが付かなくなるので、週の平均に「献立のある日 N 日」を必ず添える。**
3日のはずが2日になっていれば、そこで気付く。
...
**土日で目玉が入れ替わる号がある。** 実物を見ると「9/12 土 限り」と「9/13 日 限り」
で安い肉も野菜も丸ごと別だった。どちらに行くかで献立が変わるので、買い物日を
決めないと献立が決まらない。

### 3日ぶんは「3回の提案」ではなく「1回の買い物の割り付け」

日を独立に組ませると買い物リストがばらけて、1回の買い物で回らなくなる。月曜に
豚こま、水曜に鶏もも、金曜に鮭を別々に買うなら、チラシを読んだ意味がない。
...
これは減らしたことで出てきた問題。特売は「若鶏もも肉 4枚以上 100g当り89円」の
ように**パック単位**で安い。5日なら1kgを使い切れたが、3日だと2食ぶん（660g）しか
要らず、340g が余る。

余りは**献立の外に置く**。買い物リストには買う量をそのまま書き、使い切れない分は
冷凍して翌週へ回す。無理に3日へ詰め込むと、1食あたりの量が増えて目標から外れる。

### 土日に買って、金曜に食べるまで6日ある

作り置きはしない。土日の作業は**小分け冷凍だけ**。3日に減らしても最終日が金曜で
あることは変わらないので、冷凍は前より効く。
...
30分で作る・作り置きしない・水と金は冷凍肉、が揃うと、**前夜に冷蔵庫へ移す作業が
献立の一部**になる。金曜の夕方に凍った肉を見つけた時点で、その日の献立は成立
しない。
...
土日買い物・作り置きなし・月水金だと、自然にこうなる。

| | |
|---|---|
| 月 | 魚（買った翌日） |
| 水 | 生の肉の限界。冷凍から回すほうが安全 |
| 金 | 冷凍しておいた肉、根菜、乾物 |

**この並びを型として固定はしない。** 毎週同じ形になって飽きる。傷みやすさの規則
だけ渡して、並びは結果として決まるようにする。特売が魚だらけの週は、月と水が魚に
なっていい。
...
**代替を食べた日は、その代替に押す。** 本命と入れ替える口は置かない。
`unique (plan_id, day, slot)` があるので2行の slot を入れ替えるには一時的な値が
要るし、そもそも記録は実際に食べたものを指せばよい。
...
出どころ（`source`）を持たせて画面で区別する。ここを曖昧にすると、Phase 1 で
積み上げた「合計を信用しすぎない」が静かに壊れる。

**いまの `source` は `manual` / `plan` の2値だが、3つ要る。** `lib/food` の成分表から
計算した値は、モデルの当て推量とは別物で、手入力より信頼できることもある。
2値のままだと、その2つが同じ `plan` に潰れる。
...
create table meal_plans (
  ...
  ends_on      date not null,   -- 金曜（作らない日を含んだ週の幅）
...
create table meal_plan_items (
  ...
  day          date not null,   -- 作る日だけ行を置く。作らない日は行が無い
...
create table shopping_items (
  ...
  amount_yen integer,                   -- 2人分・3日ぶんの見込み
  to_freeze  boolean not null default false,   -- freeze は予約語なので使えない
  created_at timestamptz not null default now()
);
...
モデルに別途出させる。導出だと「鶏もも 1kg を水と金で使い切る」の整合をこちら側で
...
モデルに kcal・PFC を出させない。`lib/food/`（`food` リポジトリから写した274食品の
成分表と、名前の突き合わせ・分量のグラム換算・集計）に、`meal_plan_item_ingredients`
の `name` と `qty` をそのまま渡して計算する。

**目分量との差は大きい。** 同じ料理で 545 kcal・脂質 14g と見積もったものが、計算
すると 638 kcal・脂質 30g だった。脂質は2倍以上ずれる。

ただし計算したから正しい、ではない。274件に無い食材は近い名前に化ける（「めかぶ」が
「かぶ」に落ちた）。`calc.js` が返す `confidence` / `notes` / `counts.lowConfidence` /
`counts.unmatched` を画面に出すまでは、計算値も推定と同じ重さで扱う。
...
3,000〜5,000 トークン。出力は3日ぶん＋代替案＋買い物リストで、思考込み
8,000〜20,000 トークン。1回 30〜70 円の見込み。**これは見積りで、実測していない。**
...
- **買い物中のチェック。** `shopping_items` に「買った」を持つ列が無いので、店で
  開いても消し込めない。`bought_at` を足すのは小さいが、買い物リストだけの画面に
  分けるかどうかと一緒に決めたい
- **作らない2日をどう扱うか。** 外食した、惣菜を買った、を記録するのか、空のまま
  にするのか。何週間か使ってから決める
- **生成の引き金。** チラシを取り込んだら自動で作るか、人が押すか。チラシの発行
  周期が分かっていないので、まず押す形にする。いまは手前の一歩だけ決まっていて、
  **組むのは Claude（`.claude/skills/meal-plan`）、置くのは `/plan/import`**。
  JSON を貼ると入る（検めているのは `lib/plan-import.ts`）。アプリから API を
  呼んで組ませるのは、チラシの取り込みができてから

---

## 6. 認証と RLS

Supabase Auth のメールアドレスとパスワード。

**登録と再発行の画面は持たない。** 使うのは所帯の2人なので、人を足すのも
パスワードを直すのもダッシュボードから行う（README「入る人を増やす」）。
画面を3枚作っても一度ずつしか通らないし、使われない画面が残る（§7）。

**成否の文言を分けない。** 「そのアドレスは登録が無い」と「パスワードが違う」を
出し分けると、「このアドレスはこのサービスを使っている」を誰でも確かめられる
口になる。
