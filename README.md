...
| `/plan` | 今週の献立と買い物リスト。「食べた」で記録が1行起きる |
| `/plan/import` | 組んだ献立を JSON で貼って取り込む（下記） |
...
**献立を組むのは Claude**（`.claude/skills/meal-plan`）で、アプリは組まない。
チラシの画像を渡すと、献立と買い物リストが JSON で出てくる。それを
`/plan/import` に貼ると DB に入る。

以前はこれを手で SQL に直していた（`supabase/seed/` がその名残）。**貼るだけで
済むようにしたのが `/plan/import`。** 欄が足りない、日付が期間の外、数のつもりの
文字列、といった間違いは名前を付けて全部返すので、直してから貼り直す。

途中で失敗しても半端な行は残らない。`meal_plans` を先に `pending` で立てて
最後に `done` にしているので、落ちたらその1行を消せば下は cascade で消える。
